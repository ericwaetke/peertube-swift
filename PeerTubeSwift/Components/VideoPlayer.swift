...
private struct VideoPlayerViewControllerRepresentable: UIViewControllerRepresentable {
  ...
  class Coordinator: NSObject {
    ...
    func addObserver(to player: AVPlayer) {
      ...
      print(
        "🎬 addObserver: parent.startTime = \(String(describing: parent.startTime)), initialSeekPerformed = \(initialSeekPerformed)"
      )
      ...
      timeControlObservation = player.observe(\.timeControlStatus) { [weak self] player, _ in
        ...
        print(
          "🎬 addObserver: timeControlStatus changed to \(String(describing: player.timeControlStatus))"
        )
        ...
        if player.timeControlStatus == .playing, !self.hasNotifiedPlayerReady {
          ...
          if let startTime = self.parent.startTime, startTime > 0, !self.initialSeekPerformed {
            ...
            self.performSeekWhenReady(
              time: CMTime(seconds: Double(startTime), preferredTimescale: 600))
    ...
    func performSeekWhenReady(time: CMTime) {
      ...
      print(
        "🎬 performSeekWhenReady: Requested seek to \(time.seconds)s, status: \(item.status.rawValue)"
      )
  ...
  func updateUIViewController(_ uiViewController: AVPlayerViewController, context: Context) {
    ...
    if let req = seekRequest, context.coordinator.lastSeekRequestId != req.id {
      ...
      context.coordinator.performSeekWhenReady(
        time: CMTime(seconds: Double(req.time), preferredTimescale: 600))
    }
