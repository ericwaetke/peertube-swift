...
public final class SubscriptionRepository: ObservableObject {
	...
	public func updateChannel(_ channel: VideoChannel) throws {
		try coreDataStack.performBackgroundTask { context in
			let _ = try self.createOrUpdateChannelEntity(from: channel, in: context)
			try context.saveIfNeeded()
		}
		...
		-> ChannelSubscription
	{
		...
		let channel = VideoChannel(
			...
			avatars: channelEntity.avatarURL.map { [ActorImage(path: $0, width: nil, height: nil)] }
				?? [],
			...
			support: nil,
			isLocal: false,
			banners: channelEntity.bannerURL.map { [ActorImage(path: $0, width: nil, height: nil)] }
				?? [],
	...
	private func createOrUpdateChannelEntity(
		...
		channelEntity.avatarURL = channel.primaryAvatar?.path
		channelEntity.bannerURL = channel.primaryBanner?.path
